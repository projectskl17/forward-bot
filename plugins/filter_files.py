...
async def forward_cmd(bot, message):
    ...
    if user is not None and 'target_chat' in user.keys():
        if int(user['target_chat']) == 0:
            return await message.reply_text(
                text="<b>First add your target channel ID using /set_target command !</b>"
            )
    elif user is not None and 'target_chat' not in user.keys():
        return await message.reply_text(
            text="<b>First add your target channel ID using /set_target command !</b>"
        )
    else:
    ...
    for media_name, media_type in media_types:
        buttons.append([InlineKeyboardButton(
            f"{media_name} ❌",
            callback_data=f"toggle_media#{message.from_user.id}#{media_type}"
        )])
    
    buttons.append([InlineKeyboardButton("Confirm", callback_data=f"confirm_media#{message.from_user.id}")])
    
    target_chat = await bot.get_chat(chat_id=int(user['target_chat']))
    await bot.send_message(
        chat_id=message.from_user.id,
        text=f"Select the media types you want to forward from {source_chat.title} to {target_chat.title}:",
